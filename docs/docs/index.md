...
[![asciicast](https://asciinema.org/a/SEsRYFHdL4PnjurSZH8vAA6I0.svg)](https://asciinema.org/a/SEsRYFHdL4PnjurSZH8vAA6I0)
...
Also, if data is immutable, it doesn't need source control in the same way that code does. Therefore, ***by default, the data folder is included in the `.gitignore` file.*** If you have a small amount of data that rarely changes, you may want to include the data in the repository. Github currently warns if files are over 50MB and rejects files over 100MB. Some other options for storing/syncing large data include [GCS](https://cloud.google.com/storage) with a syncing tool (e.g., [`gsutil rsync`](https://cloud.google.com/storage/docs/gsutil/commands/rsync)), [Git Large File Storage](https://git-lfs.github.com/), [Git Annex](https://git-annex.branchable.com/), and [dat](http://dat-data.com/). Currently by default, we ask for an GCS bucket and use [GSUTIL CLI](https://cloud.google.com/storage/docs/gsutil/commands/) to sync data in the `data` folder with the server.
...
You _really_ don't want to leak your GCP secret key or Postgres username and password on Github. Enough said — see the [Twelve Factor App](http://12factor.net/config) principles on this point. Here's one way to do this:
...
GOOGLE_APPLICATION_CREDENTIALS=myaccesskey
...
#### GCS CLI configuration
When using GCS to store data, a simple method of managing GCloud access is to authenticate your user with gcloud cli:

Option 1:
[gcloud init](https://cloud.google.com/storage/docs/gsutil_install#creds-sdk)
```bash
gcloud init
```
Option 2: Setting service account with
...
The Cookiecutter Data Science project is opinionated, but not afraid to be wrong. Best practices change, tools evolve, and lessons are learned. **The goal of this project is to make it easier to start, structure, and share an analysis.** [Pull requests](https://github.com/cristian-rincon/cookiecutter-data-science/pulls) and [filing issues](https://github.com/cristian-rincon/cookiecutter-data-science/issues) is encouraged. We'd love to hear what works for you, and what doesn't.

If you use the Cookiecutter Data Science project, link back to this page or [give us a holler](https://twitter.com/uncristianr) and [let us know](mailto:cristian.o.rincon.b@gmail.com)!
